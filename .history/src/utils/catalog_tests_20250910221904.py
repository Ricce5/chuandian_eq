...
def run_magnitude_test_result(dict, Mc, obs_catalog_repr='obs', obs_name='catalog', sim_name=None, plot=True):
    ...
    result = CatalogMagnitudeTestResult(
        ...
        obs_name=obs_name,
        sim_name=sim_name if sim_name is not None else forecast.name
    )
    if plot:
        result.plot()
    return result