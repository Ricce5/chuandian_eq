...
def get_time_window_mask(timestamps: torch.Tensor, window_size: float) -> torch.Tensor:
    ...
    Args:
        timestamps: shape (batch_size, seq_len)，每个元素是该步的绝对/相对时间戳，
                    要求在同一 batch 内各序列已按时间升序排序。
        window_size: 窗口长度（与 timestamps 使用相同的时间单位）。

    Returns:
        mask: shape (batch_size, seq_len, seq_len)，torch.bool 或 torch.uint8。
              True / 1 表示需要 mask（即 **不可被注意到**）。
    """
    assert timestamps.dim() == 2, "shape 应为 (B, L)"
    bsz, L = timestamps.size()

    # t_i - t_j: 正值说明 j 在 i 之前，负值说明 j 在 i 之后（未来）
    diff = timestamps.unsqueeze(2) - timestamps.unsqueeze(1)   # (B, L, L)

    # 掩蔽条件：
    # 1. diff < 0       → 未来位置
    # 2. diff > window_size → 距离当前超过窗口
    mask = (diff < 0) | (diff > window_size)

    return mask.to(torch.uint8)   # PyTorch 旧版需要 uint8；>=1.5 推荐 .bool()
