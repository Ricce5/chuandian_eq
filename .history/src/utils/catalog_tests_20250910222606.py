...
def run_number_test_result(
    ...
    """
    根据 event_counts（预测目录事件数分布）与 obs_count（观测事件数）构造并（可选）绘图 Number Test 结果。
    """
    event_counts = np.asarray(list(event_counts), dtype=float)
    if event_counts.size == 0:
        # 没有预测分布时的退化处理
        result = CatalogNumberTestResult(
            test_distribution=[],
            name='Catalog N-Test',
            observed_statistic=float(obs_count),
            quantile=(None, None),
            status='not-valid',
            obs_catalog_repr=obs_catalog_repr,
            sim_name=sim_name,
