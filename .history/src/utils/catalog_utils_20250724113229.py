...
class DummyCatalog(src.data.Catalog):
    train: src.data.InMemoryDataset
    val: src.data.InMemoryDataset
    test: src.data.InMemoryDataset
    metadata: dict
...
def trim_train_and_test(
    ...
    new_catalog = DummyCatalog(
        train=src.data.InMemoryDataset([new_train_seq]),
        val=src.data.InMemoryDataset([new_val_seq]),
        test=catalog.test,
...
def split_sequence(seq, mean_batch_size=300, max_events=30000):
    ...
    return src.data.InMemoryDataset(short_sequences)
