...
class GutenbergRichter(Distribution):
    ...
    def __init__(self, b, mag_min=2.0, mag_max=10.0):
        ...
        @property
        def ten_pow_neg_b_dM(self):
            def _calc(b):
                dM = torch.as_tensor(self.mag_max - self.mag_min, dtype=b.dtype, device=b.device)
                return torch.exp(-b * LOG10 * dM)
            return lambda b=None: _calc(self.b if b is None else b)

        @property
        def norm_denom(self):
            def _calc(b):
                ten_pow = self.ten_pow_neg_b_dM(b)()
                denom = 1.0 - ten_pow
                # 防止极端 b 值下可能出现 0
                return torch.clamp(denom, min=torch.finfo((self.b if b is None else b).dtype).eps)
            return lambda b=None: _calc(self.b if b is None else b)
