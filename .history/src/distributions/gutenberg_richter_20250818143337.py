...
class GutenbergRichter(Distribution):
    ...
    def log_likelihood(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.log_prob(x).sum(-1)

    def aic(self, x: torch.Tensor, k: int = 1) -> torch.Tensor:
        """AIC = -2*LL + 2k, GR 模型 k=1（只有 b）"""
        ll = self.log_likelihood(x)
        return -2.0 * ll + 2.0 * k

    def bic(self, x: torch.Tensor, k: int = 1) -> torch.Tensor:
        """BIC = -2*LL + k*ln(N)"""
        n = torch.as_tensor(x.numel(), dtype=self.b.dtype, device=self.b.device)
        ll = self.log_likelihood(x)
        return -2.0 * ll + k * torch.log(n)

    def ks_stat_pvalue(self, x: torch.Tensor):
        """
