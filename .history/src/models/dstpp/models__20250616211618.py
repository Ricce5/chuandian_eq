...
class Encoder_ST(BaseEncoder):
    def __init__(self, d_model, d_inner, n_layers, n_head, d_k, d_v,
        ...
        self.event_emb_temporal = self.make_mlp(input_dim=1, output_dim=d_model)
...
class Encoder_STM(BaseEncoder):
    ...
    def temporal_enc(self, time, non_pad_mask):
        position_vec = self.position_vec.to(time.device)
    ...
    def forward(self, event_loc, event_time, event_magnitude, non_pad_mask):
        # === Attention mask
        slf_attn_mask = self.build_attention_mask(event_loc)
        slf_attn_mask = slf_attn_mask[:, :, :, 0]  # retain shape compatibility
        ...
        # === Forward through parallel stacks
        outputs = self.forward_multi_stack(
            {
                "temporal": enc_output_temporal,
                "loc": enc_output_loc,
