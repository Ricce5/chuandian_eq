...
class Transformer_type(nn.Module):
    ...
    def forward(self, event_type, event_time):
        ...
        non_pad_mask_time = get_non_pad_mask(event_time)
        non_pad_mask = get_non_pad_mask_type(event_type)
        print(torch.equal(non_pad_mask, non_pad_mask_time))
        diff_mask = (non_pad_mask != non_pad_mask_time)
        print("不一致的位置个数:", diff_mask.sum().item())
        indices = torch.nonzero(diff_mask, as_tuple=False)
        print("差异位置索引:", indices)

        # print(event_time)
        # print(non_pad_mask.squeeze(2).long().sum(1).cpu())
