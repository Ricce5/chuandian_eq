...
from .Layers import EncoderLayer,TimePositionalEncoding

def get_non_pad_mask(seq,pad=Constants.PAD):
...
def get_attn_key_pad_mask(seq_k, seq_q,pad=Constants.PAD):
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.tem_enc = TimePositionalEncoding(d_model, device=device)
        # event loc embedding
    ...
    def forward(self, event_loc, event_time, non_pad_mask):
        ...
        slf_attn_mask_keypad = get_attn_key_pad_mask(event_time,event_time)
        slf_attn_mask_keypad = slf_attn_mask_keypad.type_as(slf_attn_mask_subseq)

        slf_attn_mask = (slf_attn_mask_keypad + slf_attn_mask_subseq).gt(0)

        tem_enc = self.temporal_enc(event_time)* non_pad_mask
        enc_output = self.event_emb(event_loc)* non_pad_mask
