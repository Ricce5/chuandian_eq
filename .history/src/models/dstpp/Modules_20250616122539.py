...
class ScaledDotProductMultiHeadAttention(nn.Module):
    ...
    Input/Output:
        q, k, v: [B, L, H, D]
        attn_mask: [B, H, L, S]
        padding_mask: [B, L] (optional, not used here)
    """

    def __init__(self, d_k, attn_dropout=0.1, output_attention=True):
        super().__init__()
        self.temperature = sqrt(d_k)
    ...
    def forward(self, q, k, v, attn_mask=None, padding_mask=None):
        ...
        Args:
            q, k, v: [B, L, H, D]
            attn_mask: [B, H, L, S] or None
            padding_mask: [B, L] (unused here, placeholder for API consistency)
        """
        ...
        scores = torch.matmul(q / self.temperature, k.transpose(2, 3))  # [B, H, L, S]
...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask,padding_mask=None):
