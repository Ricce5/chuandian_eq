    ...
    def forward(self, x, mask, extra_inputs=None, return_score=False):
        ...
        if x.size(-1) != self.d_model:
            raise ValueError(f"x last dim {x.size(-1)} != d_model {self.d_model}. "
                            "Make sure your input feature dim matches module d_model.")
        if mask.dim() == 3:
            mask = mask.squeeze(-1)
        mask = mask.bool()
        ...
        B, L, _ = x.shape
        H = self.n_heads
        dh = self.d_model // H   # <- 统一用 self.d_model
        ...
        # Q = seeds, K/V = x_kv
        S = self.seeds.unsqueeze(0).expand(B, self.r, self.d_model)          # (B,r,D)
        ...
        logits = logits + bias                                                  # 删掉重复加法

        alpha = self._masked_softmax(logits, mask, dim=-1)                      # (B,H,r,L)
        ctx   = torch.matmul(alpha, V)                                          # (B,H,r,dh)

        # 这里用 self.d_model 复原
        ctx = ctx.transpose(1, 2).contiguous().view(B, self.r, self.d_model)   # (B,r,D)
        out = self.out(ctx)                                                     # (B,r,D)

        if self.agg == "concat":
            pooled = out.reshape(B, self.r * self.d_model)
        else:
