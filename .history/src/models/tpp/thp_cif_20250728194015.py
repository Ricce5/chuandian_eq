...
class THP_Cif(TppCifModel):
    ...
    def compute_lambda_at_event(self, time_delta_seqs, context):
        ...
        Args:
            time_delta_seqs (tensor): [batch_size, seq_len, num_event_types].
            context (tensor): [batch_size, seq_len, hidden_size].

        Returns:
            tensor: intensity at event times.
        """
        factor_intensity_decay = self.factor_intensity_decay[None, ...]
        factor_intensity_base = self.factor_intensity_base[None, ...]
        intensity_states = factor_intensity_decay * time_delta_seqs[:, 1:, None] + self.layer_intensity_hidden(context) + factor_intensity_base
