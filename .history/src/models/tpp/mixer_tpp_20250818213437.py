    ...
    def get_magnitude_dist(self, context, predict_b: Optional[bool] = None, return_b: bool = False):
        ...
        if predict_b:
            ...
            if self.ssm_filter is not None:
                ...
                # Clamp b_pred to a reasonable range
                b_min, b_max = 0.5, 2.0
                b_pred = b_min + (b_max - b_min) * torch.sigmoid(b_delta)
        else:
            b_pred = torch.full_like(context[..., 0], float(self.richter_b))

        mag_min = torch.full_like(context[..., 0], float(self.mag_completeness))
        gr = dist.GutenbergRichter(b=b_pred, mag_min=mag_min)
        if return_b:
            return gr, b_pred
        return gr

    def compute_b_value(self, seq: Optional[src.data.Sequence] = None, predict_b: Optional[bool] = None):
        if predict_b is None:
            predict_b = self.predict_b
        ...
        if predict_b:
            b_delta = self.hypernet_mag(context).squeeze(-1)
            if self.ssm_filter is not None:
