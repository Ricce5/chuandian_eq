...
class THP(TPPModel):
    def __init__(self, args,base_model,head, device):
        ...
        self.register_buffer("time_max", torch.tensor(args.Tmax, dtype=torch.float32))
        self.register_buffer("richter_b", torch.tensor(args.Rb, dtype=torch.float32))
        self.register_buffer(
            "mag_completeness", torch.tensor(args.Mc, dtype=torch.float32)
        )
