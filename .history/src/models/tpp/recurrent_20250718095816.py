...
class RecurrentTPP(TPPModel):
    ...
    def get_context(self, batch):
        ...
        """
        print(f"batch.inter_times { torch.sum(batch.inter_times) }{batch.inter_times.shape}, {batch.inter_times[0,:]}")
        print(f"batch.mag { torch.sum(batch.mag) }{batch.mag.shape}{batch.mag[0,:]}")
