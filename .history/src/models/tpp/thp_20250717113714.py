...
class THP(TPPModel):
    def __init__(self, args,base_model):
        ...
        self.register_buffer("tau_mean", torch.tensor(1.0, dtype=torch.float32)) 
        ...
        self.hypernet_time = nn.Linear(args.d_model, self.num_time_params).to(self.device)

        # RNN input features
        if self.input_magnitude:
            self.num_mag_params = 1  # (1 rate)
            self.hypernet_mag = nn.Linear(args.d_model, self.num_mag_params).to(self.device)
    ...
    def sample_with_cache(
        ...
        cache = self.base_model.transformer.init_cache()
        if past_seq is not None:
            t_start = past_seq.t_end
            time_remaining =  t_start - past_seq.arrival_times[-1]
