...
class RecurrentTPP(TPPModel):
    ...
    def get_context(self, batch):
        ...
        print(f"batch.mag { torch.sum(batch.mag*batch.input_mask[:, :, None]) }{batch.mag.shape}{batch.mag[0,:]}")
        feat_list = [self.encode_time(batch.inter_times*batch.input_mask[:, :, None])]  # 上一次事件到当前事件的时间间隔
        if self.input_magnitude:
            feat_list.append(self.encode_magnitude(batch.mag))
        features = torch.cat(feat_list, dim=-1).contiguous() * batch.input_mask[:, :, None]
        print(f"rnn_in { torch.sum(features*batch.input_mask[:, :, None]) }{features.shape}")
