...
class THP_Cif(TppCifModel):
    ...
    def compute_lambda_at_event(self, time_delta_seqs, context):
        factor_intensity_decay = self.factor_intensity_decay[None, ...]
        factor_intensity_base = self.factor_intensity_base[None, ...]
        intensity_states = factor_intensity_decay * time_delta_seqs[:,:,None] + self.layer_intensity_hidden(context) + factor_intensity_base
        # [batch_size, seq_len, num_event_types]
        return self.softplus(intensity_states)

    def nll_loss(self, batch):
        context = self.get_context(batch)
        time_delta_seqs, type_seq, seq_mask = masked_select_per_row([batch.inter_times,batch.type_seq,batch.non_pad_mask], batch.nll_event_mask)
        lambda_at_event = self.compute_lambda_at_event(time_delta_seqs[:,1:], context[:,1:])
        #
        sample_dtimes = self.make_dtime_loss_samples(time_delta_seqs[:, 1:])
        state_t_sample = self.compute_states_at_sample_times(event_states=self.forward(context[:,1:]), sample_dtimes=sample_dtimes)
