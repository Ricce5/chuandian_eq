...
class THP(TPPModel):
    ...
    def set_attn_type(self, new_type: str):
        self.transformer.set_attn_type(new_type)
    def set_attn_dropout(self, p: float):
        self.transformer.set_attn_dropout(p)
    
    def configure_optimizers(self):
        from torch.optim import AdamW
        optimizer = AdamW(self.parameters(), lr=self.learning_rate, weight_decay=1e-5)
        warm_up_ratio = 0.1
        step_step_lr_step_size_ratio = 0.3
        step_lr_gamma = 0.8

        if self.trainer is not None and self.trainer.estimated_stepping_batches is not None:
            total_steps = self.trainer.estimated_stepping_batches
        else:
           raise ValueError("Trainer or estimated_stepping_batches is not set. Cannot determine total steps.")

        warmup_steps = int(warm_up_ratio * total_steps)
        step_size = int( step_step_lr_step_size_ratio * total_steps)
        gamma = step_lr_gamma
        ...
        return {
            ...
            "lr_scheduler": {
                "scheduler": scheduler,
                "interval": "step",  # 每一步调用 scheduler.step()
                "frequency": 1,
            }
        }
    ...
    def sample(
        ...
        if past_seq is not None:
            ...
            buffer_batch = eq.data.Batch.init_sample_batch(past_seq=past_seq, batch_size=batch_size, max_sample_len=700)
    ...
    def sample_with_cache(
        ...
        if past_seq is not None:
            t_start = past_seq.t_end
            time_remaining =  t_start - past_seq.arrival_times[-1]
            buffer_batch = eq.data.Batch.init_sample_batch(past_seq=past_seq, batch_size=batch_size, max_sample_len=1000)
            sample_batch = buffer_batch.get_sample_batch()
...
class InputAdapter:
    ...
    def __call__(self, bx: eq.data.Batch) -> dict:
...
class BaseModel(nn.Module):
    def __init__(self, encoder, input_adapter, device: torch.device):
        super().__init__()
        self.encoder = encoder.to(device)
        self.input_adapter = input_adapter
        self.device = device

    def _move_batch(self, batch: "eq.data.Batch") -> "eq.data.Batch":
    ...
    def forward(
        ...
        features = self.input_adapter(batch)
        out, non_pad_mask, new_cache = self.encoder(features, caches=caches)
