...
class THP(TPPModel):
    def __init__(self, args,base_model,head, device):
        ...
        self.register_buffer("tau_mean", torch.tensor(tau_mean, dtype=torch.float32))  # 平均事件间隔
        ...
        self.register_buffer("time_max", torch.tensor(time_max, dtype=torch.float32))
        ...
        self.transformer = Transformer(
             d_model=context_size, d_rnn=context_size, d_inner=2*context_size,
                 n_layers=1, n_head=4, d_k=context_size//2, d_v=context_size//2,dropout_post_rnn=dropout_proba,
                 dropout=0.1, device=None, dim=1, attn_type='flash')
