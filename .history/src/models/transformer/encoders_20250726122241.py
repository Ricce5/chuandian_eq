...
from .layers import EncoderLayer,TimePositionalEncoding, RNN_layers,LocalConv
...
@BaseEncoder.register("Encoder_Conv")
class Encoder_conv(BaseEncoder):
    ...
    def __init__(self,
        ...
        self.temporal_enc = TimePositionalEncoding(d_model, device=device)
        self.event_emb = self.make_mlp(input_dim=dim, output_dim=d_model,num_layers=emb_n_layer)
        self.conv = LocalConv(d_model=d_model, siren_hid=32, 
                            siren_hid_num=2, num_channel=d_model, 
                            horizon=[5,5], omega=1)
    ...
    caches: Optional[List[Dict[str, torch.Tensor]]] = None
        ...
        if torch.isnan(out).any():
            ...
            print("stats:", out.mean(), out.std(), out.min(), out.max())
            raise ValueError("event_emb 输出包含 NaN")
        enc_output = self.conv(enc_output, event_time, ~non_pad_mask.bool()) 
        enc_output += tem_enc  # 注入时间偏置
        # === 注入时间偏置进行注意力处理 ===
        outputs = self.forward_multi_stack(
            inputs_dict={
...
class Encoder_type(BaseEncoder):
    ...
    def forward(
            self, 
            event_type, event_time, non_pad_mask,
            attn_mask: Optional[torch.Tensor] = None,
            caches: Optional[List[Dict[str, torch.Tensor]]] = None):
