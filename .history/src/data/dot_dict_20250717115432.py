...
class DotDict:
    ...
    def __init__(self, data: Optional[dict] = None, **kwargs):
        self.__dict__["_data"] = {}   # __dict__是一个特殊的属性，是一个字典，存储对象的属性
        if data is not None:
            self._data.update(data)   # 从字典初始化
        if kwargs:             
            self._data.update(kwargs) # 从关键字参数初始化
    ...
    def apply_(self, function):   # function用于所有的value
    ...
    def detach(self):
        """Detach all tensors in the DotDict."""
        return self.apply_(lambda x: x.detach() if isinstance(x, torch.Tensor) else x)
