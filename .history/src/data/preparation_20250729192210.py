    ...
    if use_double_precision:
        ...
        for cat in (catalog_ds.train, catalog_ds.val, catalog_ds.test):
            for seq in cat:
                seq.float()
        args.precision = 32
    num_events_train = sum(seq.num_nll_events for seq in catalog_ds.train)
    num_events_val = sum(seq.num_nll_events for seq in catalog_ds.val)
    print(f"Number of training events: {num_events_train}")
    print(f"Number of validation events: {num_events_val}")
