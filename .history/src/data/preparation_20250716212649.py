...
def prepare_data_tpp(args, base_dir):
    ...
    args.tau_mean = torch.cat([seq.inter_times[:-1] for seq in catalog.train]).mean().item()
    args.mag_mean = torch.cat([seq.mag for seq in catalog.train]).mean().item()
    args.time_max = torch.max(torch.tensor([seq.t_end for seq in catalog.train])).item()
    args.mag_completeness = catalog.metadata["mag_completeness"]
    if "richter_b" in catalog.metadata:
        # Use ground truth value, if available
        args.richter_b_mle = catalog.metadata["richter_b"]
    else:
