...
# 替换你原来的 catalog_from_events 函数


def catalog_from_events(events, start_time, end_time, min_mag):
    """
    用最稳妥的方法手工构造 CSEPCatalog，避免 from_dict 对 region 的要求。
    events: 列表，元素形如 {"time": "YYYY-MM-DDTHH:MM:SS", "magnitude": float}
    """
    ...
    try:
        ...
        # 转成 numpy 数组（PyCSEP 里通常用 datetime64 存时间）
        origin_time = np.array(
            [datetime.fromisoformat(e["time"]) for e in events],
            dtype='datetime64[ns]'
        )
        magnitude = np.array([e["magnitude"] for e in events], dtype=float)

        # 直接写入底层数据（不同版本字段名可能略有差异，这个最通用）
        cat._data = {
            "origin_time": origin_time,
            "magnitude": magnitude,
            # 如果你还有其它字段（如 event_id），也可加进来
        }
        ...
        raise RuntimeError(f"Failed to build CSEPCatalog: {e}")
