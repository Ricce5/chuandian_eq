...
# ---------------- 使用示例 ----------------
# seq = Sequence(..., mag=mag_tensor)   # 你的 Sequence，字段名假设为 "mag"
# updater = BayesianGRBUpdater(Mc=2.5, delta=0.97, a0=1e-3, s0=1e-3, mag_key="mag", write_back=True)
# results = updater.fit(seq)            # 逐事件更新；同时把结果字段写回 seq
# BayesianGRBUpdater.plot(seq)          # 绘图（可选传入 truth_lines / switch_index）
# # 在线追加一个事件：
# updater.update_one(3.2)               # 返回该事件后的摘要

# %%
# 假设你已有 seq = Sequence(..., mag=mag_tensor)
res = update_bayes_gr_over_sequence(
    seq, Mc=2.5, delta=0.97, a0=1e-3, s0=1e-3, mag_key="mag", write_back=True
)

# 现在 seq 里多了：
# seq["b_mean"], seq["b_sd"], seq["b_lo"], seq["b_hi"], seq["a_t"], seq["s_t"], seq["used_mask"]
# 这些张量的长度都是 seq.num_events，可直接可视化：
# plt.plot(seq.b_mean.cpu()); plt.fill_between(range(len(seq)), seq.b_lo.cpu(), seq.b_hi.cpu(), alpha=0.3)
