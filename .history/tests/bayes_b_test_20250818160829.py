...
class BayesianGRBUpdater:
    ...
    def plot(
        ...
        ax.fill_between(xs, b_lo, b_hi, alpha=0.999, label="~95% credible band")
...
# ---------------- 使用示例 ----------------
# %%
updater = BayesianGRBUpdater(Mc=3, delta=0.6, a0=5, init_b_target=0.8, mag_key="mag", write_back=True)
...
dist = GutenbergRichter(b=batch.b_mean, mag_min=2.0, mag_max=10.0)

# %%
dist.log_likelihood(batch.mag,batch.nll_event_mask.bool())
# %%
batch.t_end - batch.t_nll_start 
# %%
dist.log_likelihood(batch.mag,batch.nll_event_mask.bool())/batch.t_end - batch.t_nll_start 
# %%
dist_constant = GutenbergRichter(b=0.8, mag_min=2.0, mag_max=10.0)
