...
def _parse_profiles(raw_profiles: str, config_path: Path):
...
def _extract_best_from_summary(summary_path: Path):
    ...
    return {
        ...
        "best_value": _safe_float(best_trial.get("value")),
        "best_rmse": _safe_float(best_metrics.get("RMSE")),
        "best_dtw": _safe_float(best_metrics.get("DTW")),
        "best_dtw_normalized": _safe_float(best_metrics.get("DTW_normalized")),
        "best_params": best_trial.get("params", {}),
    }


def main():
    parser = argparse.ArgumentParser(description="One-click run LSTM Optuna for global/window profiles and aggregate best results.")
    parser.add_argument("--model", type=str, default="lstm")
    parser.add_argument("--config", type=str, default="config/lstm.yaml")
    parser.add_argument("--profiles", type=str, default="auto", help="Comma-separated profile names, or 'auto' to read from config.")
    parser.add_argument("--out_dir", type=str, default="tmp/lstm_optuna_profiles")
    ...
    profiles = _parse_profiles(args.profiles, config_path)
    ...
    for profile in profiles:
        ...
        ret = _run_cmd(cmd, repo_root)
        summary_path = profile_dir / f"optuna_summary_{profile}.json"
        parsed = _extract_best_from_summary(summary_path)
