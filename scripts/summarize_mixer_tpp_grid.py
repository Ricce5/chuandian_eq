#!/usr/bin/env python3
"""Summarize metrics from run_mixer_tpp_grid.py experiments.

This script scans one experiment directory (e.g. experiments/mixer_tpp_ablation_v1),
collects per-run test metrics, and exports:
1) per-run table (one row per seed/run),
2) grouped mean/std table by variant (or matrix key fallback),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize metrics for mixer_tpp_grid experiments.")
    parser.add_argument(
        "--exp_dir",
        type=str,
        required=True,
        help="Experiment directory produced by run_mixer_tpp_grid.py",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Metric name used to select best run in each group.",
    )
    parser.add_argument(
        "--best_mode",
        type=str,
...
def main():
    ...
    per_run_csv = out_dir / "mixer_tpp_grid_metrics_per_run.csv"
    group_csv = out_dir / "mixer_tpp_grid_metrics_group_mean_std.csv"
    best_csv = out_dir / "mixer_tpp_grid_metrics_group_best.csv"
    summary_json = out_dir / "mixer_tpp_grid_metrics_summary.json"
