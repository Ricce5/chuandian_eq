#!/usr/bin/env python3
"""Summarize metrics from run_oracle_grid.py experiments.

This script scans one experiment directory (e.g. experiments/oracle_multi_dataset),
collects per-run test metrics, and exports:
1) per-run table (one row per seed/run),
2) grouped mean/std table by variant (seed suffix stripped from run name),
3) grouped best table by a target metric.
"""
...
def group_rows(rows: Sequence[Dict], metric_keys: Sequence[str]) -> List[Dict]:
    ...
    def _base_row_builder(group_key, _items: Sequence[Dict]):
        return {
            "variant_name": str(group_key),
        }
...
def group_best_rows(rows: Sequence[Dict], metric: str, maximize: bool) -> List[Dict]:
    ...
    def _base_row_builder(group_key, _items: Sequence[Dict], best: Dict | None):
        row = {
            "variant_name": str(group_key),
        }
        if best is not None:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize metrics for oracle_grid experiments.")
    parser.add_argument(
        "--exp_dir",
        type=str,
        default="experiments/oracle_multi_dataset",
        help="Experiment directory produced by run_oracle_grid.py",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
...
def main():
    ...
    per_run_csv = out_dir / "oracle_grid_metrics_per_run.csv"
    group_csv = out_dir / "oracle_grid_metrics_group_mean_std.csv"
    best_csv = out_dir / "oracle_grid_metrics_group_best.csv"
    summary_json = out_dir / "oracle_grid_metrics_summary.json"
