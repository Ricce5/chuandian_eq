#!/usr/bin/env python3
"""Summarize metrics from run_etas_grid.py experiments.

This script scans one experiment directory (e.g. experiments/etas_ablation),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize metrics for etas_grid experiments.")
    parser.add_argument(
        "--exp_dir",
        type=str,
        required=True,
        help="Experiment directory produced by run_etas_grid.py",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
...
def main():
    ...
    per_run_csv = out_dir / "etas_grid_metrics_per_run.csv"
    group_csv = out_dir / "etas_grid_metrics_group_mean_std.csv"
    best_csv = out_dir / "etas_grid_metrics_group_best.csv"
    summary_json = out_dir / "etas_grid_metrics_summary.json"
