...
def train(data_loader, model, criterion, optimizer,scheduler, device, threshold=0.5):
    ...
    for batch, (f,mag, y) in enumerate(tqdm(data_loader, desc="Training")):
        f,mag, y = f.to(device).float(),mag.to(device), y.to(device)
        optimizer.zero_grad()
        pred = model(f,mag)
...
def validate(data_loader, model, criterion, device, threshold=0.5):
    ...
    with torch.no_grad():
        for batch, (f,mag, y) in enumerate(tqdm(data_loader, desc="Validating")):
            f,mag, y = f.to(device).float(),mag.to(device), y.to(device)
            pred = model(f,mag)
            loss = criterion(pred, y)
            val_loss += loss.item()
...
def test(data_loader, model, criterion, device, threshold=0.5, save_dir=None):
    ...
    with torch.no_grad():
        for batch, (f,mag, y) in enumerate(tqdm(data_loader, desc="Testing")):
            f,mag, y = f.to(device).float(),mag.to(device), y.to(device)
            pred = model(f,mag)
            loss = criterion(pred, y)
            test_loss += loss.item()
