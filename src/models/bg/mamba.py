...
class MambaBGModel(BGModel):
    ...
    Uses a small GRU to produce per-timepoint features which are
    ...
    def __init__(self, d_feature, d_state, device=None):
        ...
        # small RNN: maps input features -> hidden state
        self.mamba = Mamba(
            d_model=d_feature,
            d_state=d_state,
            d_conv=4,
        )
        # readout: project hidden state back to feature dimension so we can
        # combine with original time-series features if desired
    ...
    def intensity(self, ts_batch: DotDict, t_query=None, return_ssm_out=False) -> torch.Tensor:
        ...
        Returns:
            ...
            If return_ssm_out=True returns (ssm_out, intensity) where ssm_out is the
            readout SSM output per timepoint (B, T, F).
        """
        w = self.positive_weight().to(self.device)  # (1, F)

        ts = ts_batch.time_series.to(self.device) / 20.0  # (B, T, F)
        times = ts_batch.time_series_times.to(self.device)

        # run Mamba -> project to feature dim
        rnn_out = self.mamba(ts.contiguous())                      # (B, T, d_state)
        y = torch.nn.functional.softplus(rnn_out)     # (B, T, F)

        # optionally combine with original time-series features
        y = y * ts
        ...
        if t_query is None:
            if hasattr(ts_batch, "arrival_times") and ts_batch.arrival_times is not None:
                t_query = ts_batch.arrival_times.to(self.device)
            else:
                t_query = times
        ...
        # ensure non-negative numerical stability and return proper shape
        intensity = intensity.squeeze(-1).clamp_min(0.0)  # (B, Nq)

        if return_ssm_out:
            return rnn_out, intensity  # (B, T, F), (B, Nq)
        else:
            return intensity  # (B, Nq)
    ...
    def intensity_integral(self, batch: DotDict) -> torch.Tensor:
        ...
        """
        w = self.positive_weight().to(self.device)  # (1, F)
        ts = batch.time_series.to(self.device) / 20.0      # (B, T, F)
        times = batch.time_series_times.to(self.device)

        # use self.mamba (not self.rnn)
        rnn_out = self.mamba(ts.contiguous())                   # (B, T, d_state)
        y = torch.nn.functional.softplus(rnn_out)  # (B, T, F)
        y = y * ts
