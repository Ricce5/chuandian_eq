# Pure Ogata-parameterized ETAS implementation.
# Productivity kernel:
...
def _compute_branching_ratio_ogata(
    ...
    """Compute infinite-horizon ETAS branching ratio under Ogata parameterization.
    ...
    if branching_ratio > 1:
        logger.warning("Branching ratio (Ogata): %s", branching_ratio)
...
class ETASOgata(TPPModel):
    """ETAS model in pure Ogata parameterization.
    ...
    def __init__(
        ...
        super().__init__()
        if omori_p_init <= 1.0:
            raise ValueError("Ogata parameterization requires p > 1.")
    ...
    @property
    def p(self):
        return 1.0 + torch.exp(self.log_p_minus_one)
    ...
    def branching_ratio(self) -> float:
        """Infinite-horizon branching ratio under Ogata parameterization."""
        return float(
    ...
    def _torch_omori_inv(T1, T2, c, p, size, t_max, device, dtype):
        ...
        if torch.any(T2 < T1):
            raise ValueError("T2 must be >= T1 after clipping to [0, t_max].")
        F0 = ETASOgata._torch_omori_int(zero, tmax_t, c_t, p_t)

        F1 = ETASOgata._torch_omori_int(zero, T1, c_t, p_t) / F0
        F2 = ETASOgata._torch_omori_int(zero, T2, c_t, p_t) / F0
        u_prime = u * (F2 - F1) + F1
    ...
    def nll_loss(
        ...
        *,
        ...
        delta_t = t_select.unsqueeze(-1) - t.unsqueeze(-2)
        prev_mask = (delta_t > 0).float()

        omori = (delta_t * prev_mask + self.c).pow(-self.p)
        amp = self.K * torch.exp(self.alpha_e * (batch.mag - self.M_c))
        intensity = (
            omori
            * self.omori_norm_factor
            * amp.unsqueeze(-2)
            * prev_mask
            * survival_mask.unsqueeze(-2)
        ).sum(-1) + self.mu
        ...
        integral = (kernel_mass * amp * survival_mask).sum(-1)
    ...
    def h_intensity(self, batch: Batch, t_query: torch.Tensor = None) -> torch.Tensor:
        ...
        delta_t = t_query.unsqueeze(-1) - t.unsqueeze(-2)
        prev_mask = (delta_t > 0).float()
        omori = (delta_t * prev_mask + self.c).pow(-self.p)
    ...
    def print_params(self):
        ...
        print("ETAS (Ogata) model parameters:")
    ...
    def set_params(
        ...
        *,
        ...
        """Set ETAS parameters under a single Ogata parameterization."""
        with torch.no_grad():
            if p is not None:
                if p <= 1.0:
                    raise ValueError("Ogata parameterization requires p > 1.")
