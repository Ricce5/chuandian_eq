...
from src.models.bg.latent_bg import LatentBGModel
...
def test_latent_bg_model_is_registered():
    assert BGModel.by_name("latent_bg") is LatentBGModel


def test_latent_bg_model_is_random_in_train_mode():
    torch.manual_seed(0)
    device = get_device()
...
def test_latent_bg_model_is_deterministic_in_eval_by_default():
    torch.manual_seed(0)
    device = get_device()
    batch = make_batch(device)
...
def test_latent_bg_model_keeps_zero_input_zero_output():
    torch.manual_seed(0)
    device = get_device()
...
def test_latent_bg_model_nll_variants_are_finite_and_track_kl():
    torch.manual_seed(0)
    device = get_device()
    batch = make_batch(device)
...
def test_latent_bg_model_sequence_mode_still_works():
    torch.manual_seed(0)
    device = get_device()
    batch = make_batch(device)
